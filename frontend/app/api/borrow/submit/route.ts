...
// Phase 2, step 2 — the user has signed the prepared tx in their wallet. Forward
// it to the oracle to submit on-chain and confirm.
export async function POST(req: NextRequest) {
  const body = await req.json().catch(() => null);
  if (!body?.signedXdr) return NextResponse.json({ error: "signedXdr required" }, { status: 400 });

  try {
    const res = await fetch(`${ORACLE_URL}/borrow/submit`, {
      method: "POST",
